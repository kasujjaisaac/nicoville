...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        label { display:block; margin-bottom:8px; color:var(--ink); font-size:13px; font-weight:900; text-transform:uppercase; }
        textarea { width:100%; min-height:65vh; padding:16px; border:1px solid var(--line); background:#fbfcfa; color:var(--ink); font:14px/1.5 Consolas,monospace; resize:vertical; }
        ...
        @media (max-width:820px){ .admin-layout{grid-template-columns:1fr;} .sidebar{position:static;height:auto;} }
    </style>
</head>
<body>
    <div class="admin-layout">
        <aside class="sidebar" aria-label="Admin navigation">
            ...
            <nav class="sidebar-nav">
                ...
                <a class="sidebar-link" href="/admin/event-bookings"><span class="sidebar-icon">03</span><span>Event Bookings</span></a>
                <a class="sidebar-link active" href="/admin/content/pages" aria-current="page"><span class="sidebar-icon">04</span><span>Content Manager</span></a>
            </nav>
        ...
        <main class="admin-shell">
            ...
            <form class="editor-card" method="POST" action="/admin/content/{{ $section }}">
                @csrf
                <h1>{{ $sections[$section] }}</h1>
                <p>Edit the JSON below to update words, image URLs, button labels, button links, categories, amounts, and page content. Keep the JSON brackets, commas, and quotes valid.</p>
                <div class="hint">Images can use uploaded paths such as <strong>/uploads/slides/slide-name.jpg</strong> or a full image URL. For repeated items, duplicate one object and change its values.</div>
                <label for="content">Editable Content JSON</label>
                <textarea id="content" name="content" spellcheck="false" required>{{ old('content', $content) }}</textarea>
                <div class="actions">
